...
function animazione_pushdown_separata(t1, P1_1, P2_1, P3_1, theta1_1, theta2_1, J1, tau1, ...
    ...
    uitblAreaForza = uitable('Data',{'--','--'},'ColumnName',{'errata','corretta'},...
        'Position',[20 490 200 48],'FontSize',12,'BackgroundColor',[1 1 0.95]);
    uicontrol('Style','text','String','Area ellisse di FORZA [N^2]:','Position',[20 540 300 40],'FontSize',12,'BackgroundColor',[1 1 0.95]);

    % Tabella per lunghezza diagonale maggiore ellisse di forza
    uitblDiag = uitable('Data',{'--','--'},'ColumnName',{'errata','corretta'},...
        'Position',[20 430 200 48],'FontSize',12,'BackgroundColor',[0.95 1 0.95]);
    uicontrol('Style','text','String','Lunghezza diagonale maggiore ellisse di FORZA [N]','Position',[20 480 350 40],'FontSize',12,'BackgroundColor',[0.95 1 0.95]);
    ...
    function aggiornaEllisse(frame)
        ...
        if ~isnan(lung1)
            diagData{1,1} = sprintf('%.2f', lung1);
        else
            diagData{1,1} = '--';
        end
        if ~isnan(lung2)
            diagData{1,2} = sprintf('%.2f', lung2);
        else
            diagData{1,2} = '--';
        end
